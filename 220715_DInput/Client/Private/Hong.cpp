...
HRESULT CHong::Render()
{
	...
	const char* TexIndex[] =
	{
		...
		"WallSpr_12","WallSpr_13","WallSpr_14","WallSpr_15","WallSpr_16"
...
HRESULT CHong::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	...
	if (FAILED(pGameInstance->Add_GameObjectToLayer(TEXT("Prototype_GameObject_Terrain"), LEVEL_HONG, pLayerTag, vPos)))
		return E_FAIL;
	vPos.x += 29.f;
	if (FAILED(pGameInstance->Add_GameObjectToLayer(TEXT("Prototype_GameObject_Terrain"), LEVEL_HONG, pLayerTag, vPos)))
		return E_FAIL;
