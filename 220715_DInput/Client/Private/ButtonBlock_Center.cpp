...
HRESULT CButtonBlock_Center::Initialize(void* pArg)
{
	...
	if (pArg != nullptr)
	{
		...
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, _float3(21.f, 1.f, 3.f));
...
void CButtonBlock_Center::SetDead()
{
	m_bActive = false;
	//Particle Effect
	//for (int i = 0; i < 7; i++)
...
HRESULT CButtonBlock_Center::SetUp_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_SENI, TEXT("Prototype_Component_Texture_ButtonBlock_Center"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom, this)))
		return E_FAIL;
